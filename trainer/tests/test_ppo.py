...
def test_recurrent_ppo_update_runs_on_padded_sequences():
    ...
    for tick in range(5):
        transitions.append(
            Transition(
                ...
                categorical_action={
                    name: int(value[0]) for name, value in action.categorical.items()
                },
                camera_action=np.zeros(2, dtype=np.float32),
                old_log_probability=-5.0,
                old_value=0.0,
                reward=0.01,
                done=tick == 4,
                next_value=0.0,
    ...
    trainer = PPOTrainer(
        policy, PPOConfig(optimization_epochs=1, minibatch_samples=8), torch.device("cpu")
    )
