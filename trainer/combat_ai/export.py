...
def export_onnx(policy: CombatPolicy, destination: Path) -> None:
    ...
    inputs = (
        torch.zeros(1, SELF_SIZE, device=device),
        torch.zeros(1, OPPONENT_SIZE, device=device),
        torch.zeros(1, 1, device=device),
        torch.zeros(1, MAX_ENTITIES, ENTITY_SIZE, device=device),
        torch.zeros(1, MAX_ENTITIES, device=device),
        torch.zeros(1, MAX_BLOCKS, BLOCK_SIZE, device=device),
        torch.zeros(1, MAX_BLOCKS, device=device),
        torch.zeros(1, LEGAL_SIZE, device=device),
...
def export_flat_weights(policy: CombatPolicy, manifest_path: Path, weights_path: Path) -> None:
    ...
    for name, value in policy.state_dict().items():
        ...
        entries.append(
            {"name": name, "shape": list(array.shape), "offset_f32": offset, "length": array.size}
        )
    ...
    manifest_path.write_text(
        json.dumps(
            {
                "format": "mcai-flat-f32",
                "format_version": 1,
                "little_endian": True,
                "parameter_count": int(packed.size),
                "architecture": "structured-mlp-gru128-v1",
                "categorical_heads": CATEGORICAL_SIZES,
                "tensors": entries,
            },
            indent=2,
        )
