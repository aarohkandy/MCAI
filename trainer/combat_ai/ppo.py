...
class PPOTrainer:
    ...
    def _update(self, batch: SequenceBatch) -> UpdateMetrics:
        ...
        return UpdateMetrics(
            policy_loss=averaged[0],
            value_loss=averaged[1],
            entropy=averaged[2],
            approximate_kl=averaged[3],
            clip_fraction=averaged[4],
            gradient_norm=averaged[5],
    ...
    def _minibatch(self, batch: SequenceBatch) -> tuple[float, ...]:
        ...
        clipped = (
            ratio.clamp(1 - self.config.clip_ratio, 1 + self.config.clip_ratio) * batch.advantage
        )
        ...
        value_error = torch.maximum(
            (value - batch.returns).square(), (clipped_value - batch.returns).square()
        )
    ...
    def auxiliary_imitation_update(
        self, records: list[dict], weight: float, batch_size: int = 512
    ) -> float:
...
def _is_mps_unsupported(error: RuntimeError) -> bool:
    ...
    return "mps" in message and any(
        word in message for word in ("unsupported", "not implemented", "placeholder")
    )
...
def _batch_to(batch: SequenceBatch, device: torch.device) -> SequenceBatch:
    return SequenceBatch(
        ...
        actions=type(batch.actions)(
            categorical={
                name: value.to(device) for name, value in batch.actions.categorical.items()
            },
            camera=batch.actions.camera.to(device),
        ),
        old_log_probability=batch.old_log_probability.to(device),
