...
def main() -> None:
    ...
    browser = subparsers.add_parser(
        "browser", help="serve frozen JSON inference to the Eagler adapter"
    )
    ...
    curriculum = subparsers.add_parser(
        "curriculum", help="check and persist a curriculum evaluation gate"
    )
    curriculum.add_argument("--state", type=Path, default=Path("checkpoints/curriculum.json"))
    curriculum.add_argument(
        "--results", type=Path, help="JSON containing stage and named held-out metrics"
    )
    league = subparsers.add_parser(
        "league-eval", help="record held-out Elo and detect a five-evaluation plateau"
    )
    league.add_argument("held_out_elo", type=float)
    league.add_argument("--checkpoints", type=Path, default=Path("checkpoints"))
    promote = subparsers.add_parser(
        "promote-exploiter", help="atomically add an exploiter to the main frozen pool"
    )
    ...
    if arguments.command == "serve":
        ...
        service = ServiceConfig(
            host=arguments.host,
            port=arguments.port,
            checkpoint_dir=arguments.checkpoints,
            deterministic_inference=arguments.deterministic,
        ...
        asyncio.run(
            serve(
                ppo,
                service,
                arguments.imitation_data,
                arguments.initialize_from,
                arguments.exploiter_target,
            )
        )
        ...
        export_flat_weights(
            policy,
            arguments.directory / "policy.manifest.json",
            arguments.directory / "policy.weights.bin",
        )
        ...
        print(
            json.dumps({"held_out_elo": arguments.held_out_elo, "exploiter_requested": requested})
        )
