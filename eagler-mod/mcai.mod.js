;(function MCAIEaglerAdapter(global) {
  ...
  API.meta.description(
    'Structured-state combat policy adapter using only ordinary game controls. F8 is emergency stop.'
  )
  ...
  const config = Object.assign(
    {
      bridgeUrl: 'ws://127.0.0.1:8767',
      enabled: false,
      overlay: true,
      episodeId: 'browser-session',
      arenaSeed: 0,
      actionDelayTicks: 0,
  ...
  function initialize() {
    try {
      reflected.BlockPos =
        API.reflect.getClassById('net.minecraft.util.math.BlockPos') ||
        API.reflect.getClassById('net.minecraft.util.BlockPos')
      reflected.BlockPosConstructor =
        reflected.BlockPos.constructors.find(constructor => constructor.length === 3) ||
  ...
  function buildObservation(player) {
    ...
    return {
      ...
      match: {
        ...
        arena_seed: config.arenaSeed,
        action_delay_ticks: config.actionDelayTicks,
      ...
      self: {
        ...
        yaw: canonicalYaw(player.rotationYaw),
        pitch: canonicalPitch(player.rotationPitch),
        on_ground: bool(player.onGround),
        sprinting: callBoolean(player, 'isSprinting', false),
        sneaking: callBoolean(player, 'isSneaking', false),
        hurt_time: number(player.hurtTime, 0),
        attack_cooldown: attackCooldown(player),
        active_hand: activeHand(player),
        use_ticks: state.activeHand === 'none' ? 0 : state.tick - state.useStartedTick,
        mining_progress: miningProgress(),
        selected_hotbar: number(player.inventory && player.inventory.currentItem, 0),
        hotbar: hotbar(player),
        offhand: itemState(call(player, 'getHeldItemOffhand')),
        armor: armor(player),
  ...
  function opponentState(player, opponent, selfPosition, selfVelocity) {
    ...
    return {
      ...
      yaw: canonicalYaw(opponent.rotationYaw),
      pitch: canonicalPitch(opponent.rotationPitch),
      health: hasMethod(opponent, 'getHealth') ? callNumber(opponent, 'getHealth', 0) : null,
      hurt_time: number(opponent.hurtTime, 0),
      on_ground: bool(opponent.onGround),
      line_of_sight: canSee(player, opponent),
      mainhand: itemState(call(opponent, 'getHeldItemMainhand')),
      offhand: itemState(call(opponent, 'getHeldItemOffhand')),
      armor: armor(opponent)
    }
  }
  ...
  function entitySlots(player, selfPosition, selfVelocity) {
    ...
    for (const entity of javaList(corrected(API.world).loadedEntityList)) {
      ...
      slots.push({
        ...
        distance: distance(selfPosition, entityPosition),
        raycastable: referenceEquals(pointed, entity)
      })
    }
  ...
  function blockSlots(player, selfPosition, opponentPosition) {
    ...
    for (const center of centers) {
      const bx = Math.floor(center.x),
        by = Math.floor(center.y),
        bz = Math.floor(center.z)
      for (let dy = -2; dy <= 3; dy++)
        for (let dx = -5; dx <= 5; dx++)
          for (let dz = -5; dz <= 5; dz++) {
            ...
            const clearance =
              base &&
              blockReplaceable(blockAt(world, offset(coordinates, 0, 1, 0))) &&
            ...
            candidates.push({
              name,
              relative_position: egocentric(subtract(coordinates, selfPosition), canonicalYaw(player.rotationYaw)),
              collision: block.collision,
              hardness: block.hardness,
              replaceable,
              break_progress: pointedAt ? miningProgress() : 0,
              crystal_clearance: clearance,
              exposed_faces: faces,
              distance: dist,
              within_reach: dist <= 5,
              raycastable: pointedAt,
              sample_age_ticks: 0,
              score: dist - (clearance ? 4 : base ? 2 : 0) - (pointedAt ? 8 : 0)
            })
          }
    }
    state.blockCache = candidates
      .sort(
        (a, b) =>
          a.score - b.score ||
  ...
  function blockAt(world, coordinates) {
    try {
      ...
      let hardness = 0
      try {
        hardness = number(block.getBlockHardness(world.getRef(), rawPosition), 0)
      } catch (_) {
      ...
      return {
        ...
        name,
        replaceable,
        collision: liquid ? 'liquid' : replaceable ? 'empty' : solid ? 'solid' : 'partial',
        hardness
      }
    } catch (_) {
      return null
  ...
  function blockReplaceable(block) {
    return !block || block.replaceable || block.collision === 'empty'
  }
  ...
  function actionMask(player) {
    ...
    return {
      ...
      release_use: state.activeHand !== 'none',
      swap_offhand: main.count > 0 || offhand.count > 0,
  ...
  function applyAction(player, action) {
    ...
    player.rotationYaw =
      number(player.rotationYaw, 0) + Coordinates.minecraftYawDelta(clamp(action.yaw_delta, -Math.PI, Math.PI))
    player.rotationPitch = clamp(
      number(player.rotationPitch, 0) + Coordinates.minecraftPitchDelta(action.pitch_delta),
      -90,
      90
    )
  ...
  function recordHumanAction(observation, player) {
    const settings = corrected(API.settings)
    const yaw = canonicalYaw(player.rotationYaw),
      pitch = canonicalPitch(player.rotationPitch)
    ...
    const action = {
      ...
      jump: keyPressed(settings, 'keyBindJump'),
      sprint: keyPressed(settings, 'keyBindSprint'),
      sneak: keyPressed(settings, 'keyBindSneak'),
      yaw_delta: normalizeAngle(yaw - state.lastYaw),
      ...
      release_use: !use && state.activeHand !== 'none',
      hotbar: selected === state.lastHotbar ? -1 : selected,
  ...
  function ensureBridge() {
    ...
    try {
      ...
      socket.onerror = () => {
        state.error = 'Local policy bridge unavailable'
        state.pending = false
      }
    } catch (error) {
  ...
  function delayedObservation(current) {
    ...
    return {
      ...
      blocks: delayed.blocks.map(block => ({
        ...block,
        sample_age_ticks: block.sample_age_ticks + config.observationDelayTicks
      }))
    }
  }
  ...
  function chatCommand(event) {
    ...
    if (command === 'on') {
      config.enabled = true
      API.displayToChat('MCAI enabled (structured state, unrestricted legal timing).')
      ...
      state.demonstration = []
      state.matchId = parts[2] || makeId()
      config.episodeId = state.matchId
      API.displayToChat(`Recording demonstration ${state.matchId}.`)
    } else if (command === 'recordstop') {
      state.recording = false
      API.displayToChat(`Recorded ${state.demonstration.length} ticks.`)
      ...
      state.currentAction = noopAction()
      API.displayToChat(
        `MCAI delays set to action=${config.actionDelayTicks}, observation=${config.observationDelayTicks} ticks.`
      )
    } else
      API.displayToChat(
        `MCAI ${config.enabled ? 'ON' : 'OFF'}, bridge=${state.connected}, recording=${state.recording}, ticks=${state.demonstration.length}`
      )
  }

  function exportDemonstration() {
    if (!state.demonstration.length) {
      API.displayToChat('No demonstration frames to export.')
      return
    }
    ...
    anchor.href = url
    anchor.download = `mcai-demo-${state.matchId}.jsonl`
    anchor.click()
  ...
  function installOverlay() {
    ...
    element.style.cssText =
      'position:fixed;right:10px;top:10px;z-index:2147483647;padding:8px 10px;' +
  ...
  function updateOverlay(player) {
    ...
    const target = state.target
      ? `${number(state.target.x, 0).toFixed(1)},${number(state.target.y, 0).toFixed(1)},${number(state.target.z, 0).toFixed(1)}`
      : 'none'
  ...
  function updateRewardComponents(observation) {
    ...
    const opponentDamage =
      state.lastOpponentHealth === null || opponentHealth === null
        ? 0
        : 0.02 * Math.max(0, state.lastOpponentHealth - number(opponentHealth, state.lastOpponentHealth))
  ...
  function nearestOpponent(player) {
    ...
    return (
      javaList(corrected(API.world).playerEntities)
  ...
  function itemState(stackValue) {
    ...
    return {
      ...
      count: Math.max(0, Math.trunc(number(stack.count, number(stack.stackSize, 1)))),
      durability: maximum > 0 ? Math.max(0, maximum - used) : 0,
  ...
  function raycast(player) {
    ...
    if (entity)
      return {
        kind: 'entity',
        distance: distance(eyePosition(player), position(entity)),
        block_name: '',
    ...
    if (blockPosition) {
      ...
      return {
        kind: 'block',
        distance: distance(eyePosition(player), blockPosition),
  ...
  function sameBlockPos(positionValue, expected) {
    return (
      !!positionValue &&
      number(positionValue.x, NaN) === expected.x &&
      number(positionValue.y, NaN) === expected.y &&
      number(positionValue.z, NaN) === expected.z
    )
  }

  function activeHand(player) {
    if (!callBoolean(player, 'isHandActive', false)) {
      state.activeHand = 'none'
      return 'none'
    }
    try {
  ...
  function attackCooldown(player) {
    try {
      return clamp(number(player.getCooledAttackStrength(0), 0), 0, 1)
    } catch (_) {
      return clamp((state.tick - state.lastAttackTick) / 13, 0, 1)
    }
  }
  ...
  function keyPressed(settings, name) {
    try {
      return bool(corrected(settings && settings[name]).pressed)
    } catch (_) {
      return false
    }
  }
  ...
  function position(entity) {
    return { x: number(entity.posX, 0), y: number(entity.posY, 0), z: number(entity.posZ, 0) }
  }
  function eyePosition(entity) {
    const value = position(entity)
    value.y += callNumber(entity, 'getEyeHeight', 1.62)
    return value
  }
  function velocity(entity) {
    return { x: number(entity.motionX, 0), y: number(entity.motionY, 0), z: number(entity.motionZ, 0) }
  }
  function subtract(a, b) {
    return { x: a.x - b.x, y: a.y - b.y, z: a.z - b.z }
  }
  function offset(a, x, y, z) {
    return { x: a.x + x, y: a.y + y, z: a.z + z }
  }
  function distance(a, b) {
    return Math.hypot(a.x - number(b.x, 0), a.y - number(b.y, 0), a.z - number(b.z, 0))
  }
  ...
  function canSee(player, entity) {
    try {
      return bool(player.canEntityBeSeen(entity.getRef()))
    } catch (_) {
      return false
    }
  }
  function entityKind(entity) {
    try {
      const raw = (
        javaString(call(call(entity, 'getClass'), 'getSimpleName')) ||
        javaString(call(entity, 'getName')) ||
        'unknown'
      ).toLowerCase()
  ...
  function playerName() {
    return javaString(call(corrected(API.player), 'getName')) || 'MCAI_BROWSER'
  }
  function corrected(value) {
    try {
      return value && value.getCorrective ? value.getCorrective() : value
    } catch (_) {
      return value
    }
  }
  function javaList(value) {
    ...
    try {
      return Array.from({ length: number(list.size(), 0) }, (_, index) => corrected(list.get(index)))
    } catch (_) {
      return []
    }
  }
  function javaString(value) {
    ...
    try {
      return API.util.unstring(value.getRef ? value.getRef() : value)
    } catch (_) {
      try {
        return String(value)
      } catch (_) {
  ...
  function call(value, name, ...args) {
    try {
      return value && typeof value[name] === 'function' ? corrected(value[name](...args)) : null
    } catch (_) {
      return null
    }
  }
  function callNumber(value, name, fallback) {
    return number(call(value, name), fallback)
  }
  function callBoolean(value, name, fallback) {
    const result = call(value, name)
    return result === null ? fallback : bool(result)
  }
  function referenceEquals(a, b) {
    if (!a || !b) return false
    try {
      return (a.getRef ? a.getRef() : a) === (b.getRef ? b.getRef() : b)
    } catch (_) {
      return a === b
    }
  }
  ...
  function canonicalYaw(value) {
    return Coordinates.canonicalYaw(value)
  }
  function canonicalPitch(value) {
    return Coordinates.canonicalPitch(value)
  }
  ...
  global.MCAI = Object.freeze({
    ...
    emergencyStop,
    status() {
      return { ...state, socket: undefined, flatPolicy: !!state.flatPolicy }
    },
